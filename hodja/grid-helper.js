(function(exports) {
	...
	exports.GridHelper = Object.freeze({
		...
		gridModalEdit:gridModalEdit,
		modalRowOK:modalRowOK,
		modalRowChange:modalRowChange
		// ,
	...
	function refreshGridRows(gridId,locals,editRowIndex=-1){
							...
							if(itemValue!=''){
								...
								if(remoteList[field.field].list[itemValue]==undefined){
									...
									if(field.lookupTextField){
										remoteList[field.field].list[itemValue]['lookupTextField']=`${generateFormName(parentField + '.' + rowIndex + '.' + field.lookupTextField)}`
									}
								}
							}
							td.className=field.class || 'ml-1'
		...
		if(editRowIndex<0 && !locals.modal){
			editRow(gridId,grid,locals)
		}
		if(locals.modal && editRowIndex>-1){
			gridModalEdit(gridId,locals,editRowIndex)
		}
...
function modalRowChange(modalId,locals,e){
	var fields=locals.fields
	var obj=getModalRowData(modalId,locals)
	Object.keys(fields).forEach((key)=>{
		var field=fields[key]
		if(!field.field){
			field.field=key
		}
		if(field.calc && e.target.id!=`${generateFormId(modalId + '.' + field.field)}`){
			$(`#${generateFormId(modalId + '.' + field.field)}`).val(eval(replaceUrlCurlyBracket(field.calc,obj)))
		}
	})
}

	function setModalRowData(modalId,locals,listItem){
		...
		Object.keys(fields).forEach((key)=>{
			...
			var valueText=''
			if(listItem){
				itemValue=getPropertyByKeyPath(listItem,field.field) || ''
				if(field.lookupTextField){
			...
			switch(field.type.toLowerCase()){
				...
				$(`input[name="${generateFormName(modalId + '.' + field.field)}"]`).val(itemValue)
				if(field.lookupTextField){
					$(`#${generateFormId(modalId + '.' + field.field)}-autocomplete-text`).val(valueText)
					var controlNameTextField=generateFormName(modalId + '.' + field.lookupTextField)
	...
	function gridModalEdit(gridId,locals,editRowIndex=-1){
		
		if(editRowIndex<0){
			$(`#modalRow${locals.id}Label`).html("Yeni satır")
		}else{
			$(`#modalRow${locals.id}Label`).html(`#${editRowIndex+1} satır düzenle`)
		}
		var s=`<a class="btn btn-primary" href="javascript:gridHelper.modalRowOK('${gridId}',${gridId},${editRowIndex})" title="Kaydet"><i class="fas fa-check"></i> Tamam1</a>
		<button class="btn btn-secondary" type="button" data-dismiss="modal">Vazgeç</button>`

		var listItem
		if(editRowIndex>-1){
			listItem=locals.value[editRowIndex]
		}
		if(locals.modal){
			if(locals.modal.tabs){
				locals.modal.tabs.forEach((tab)=>{
					if(tab.fields){
						setModalRowData(`modalRow${locals.id}`,tab,listItem)
		...
		$(`#modalRow${locals.id} .modal-footer`).html(s)		
		$(`#modalRow${locals.id}`).modal('show')
	}

	function getModalRowData(modalId,locals){
		var obj={}
		var fields=locals.fields
		Object.keys(fields).forEach((key)=>{
			var field=fields[key]
			...
			switch(field.type.toLowerCase()){
				case 'number':
				case 'money':
				obj[field.field]=Number($(`#${generateFormId(modalId + '.' + field.field)}`).val() || 0)
				break
				case 'remotelookup':
				obj[field.field]=$(`input[name="${generateFormName(modalId + '.' + field.field)}"]`).val()
				if(field.lookupTextField){
					var controlNameTextField=generateFormName(modalId + '.' + field.lookupTextField)
					obj[field.lookupTextField]=$(`input[name="${controlNameTextField}"]`).val()
				}
				break
				default:
				obj[field.field]=$(`#${generateFormId(modalId + '.' + field.field)}`).val() || ''
				break
			}
	...
	function modalRowOK(gridId,locals,editRowIndex=-1){
		var modalId=`modalRow${locals.id}`
		...
		if(locals.modal){
			if(locals.modal.tabs){
				...
				locals.modal.tabs.forEach((tab)=>{
					if(tab.fields){
						var obj1=getModalRowData(modalId,tab)
		...
		if(editRowIndex<0){
			locals.value.push(obj)
		}else{
			locals.value[editRowIndex]=obj
		}
		
		refreshGridRows(gridId,locals,editRowIndex)

		$(`#modalRow${locals.id}`).modal('hide')
