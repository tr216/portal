...
exports.makeSimpleInvoiceList=function(doc){
	...
	switch(doc.invoiceStatus){
		...
		case 'WaitingForApprovement': newDoc.invoiceStatus +='invoice-waitingforapprovement">Onay bekliyor';break;
...
exports.makeSimpleOrderList=function(doc){
	...
	switch(doc.orderStatus){
		...
		case 'WaitingForApprovement': newDoc.orderStatus +='invoice-waitingforapprovement">Onay bekliyor';break;
...
exports.makeSimpleDespatchList=function(doc){
	...
	newDoc.despatchStatusHtml ='<span class="badge badge-invoice ';
	switch(doc.despatchStatus){
		case 'Deleted': newDoc.despatchStatusHtml +='invoice-declined">Silindi';break;
		case 'Draft': newDoc.despatchStatusHtml +='invoice-draft">Taslak';break;
		case 'Pending': newDoc.despatchStatusHtml +='invoice-pending">Gönderiliyor';break;
		case 'Queued': newDoc.despatchStatusHtml +='invoice-processing">Kuyruğa alındı';break;
		case 'Processing': newDoc.despatchStatusHtml +='invoice-processing">İşleniyor';break;
		case 'SentToGib': newDoc.despatchStatusHtml +='invoice-senttogib">GİB\'e iletildi';break;
		case 'Approved':	newDoc.despatchStatusHtml +='invoice-approved">Onaylandı';break;
		case 'PartialApproved':	newDoc.despatchStatusHtml +='invoice-partial-approved">Kısmi Onaylandı';break;
		case 'Declined': newDoc.despatchStatusHtml +='invoice-declined">Reddedildi'; break;
		case 'Canceled': newDoc.despatchStatusHtml +='invoice-declined">İptal Edildi'; break;
		case 'WaitingForApprovement': newDoc.despatchStatusHtml +='invoice-waitingforapprovement">Onay bekliyor';break;
		case 'Error': newDoc.despatchStatusHtml +='invoice-error"><a href="javascript:showErrors(\'' + doc._id + '\');"><i class="fa fa-eye"></i> Hata</a>';break;
		default: newDoc.despatchStatusHtml +='invoice-processing">' + doc.despatchStatus + ''; break;
	}
	newDoc.despatchStatusHtml +='</span>';

	if(doc.ioType==1){
		newDoc.receiptStatusHtml ='<span class="badge badge-invoice '
		switch(doc.receiptAdvice.receiptStatus){
			case '':
			case 'Waiting':
				if(doc.despatchStatus=='Approved'){
					newDoc.receiptStatusHtml +='invoice-approved">Başarılı(auto)'
				}else{
					newDoc.receiptStatusHtml +='invoice-waitingforapprovement">Teslimat bekleniyor'
				}
				
			break
			case 'Draft': 
				newDoc.receiptStatusHtml +='invoice-draft">Taslak'
			break
			case 'Pending':
				newDoc.receiptStatusHtml +='invoice-pending">Gönderiliyor'
			break
			case 'Queued':
				newDoc.receiptStatusHtml +='invoice-processing">Kuyruğa alındı'
			break
			case 'Processing':
				newDoc.receiptStatusHtml +='invoice-processing">İşleniyor'
			break
			case 'SentToGib':
				newDoc.receiptStatusHtml +='invoice-senttogib">GİB\'e iletildi'
			break
			case 'Success':
				newDoc.receiptStatusHtml +='invoice-approved">Başarılı'
			break
			case 'Error':
				newDoc.receiptStatusHtml +='invoice-error"><a href="javascript:showReceiptErrors(\'' + doc.receiptAdvice._id + '\');"><i class="fa fa-eye"></i> Hata</a>'
			break
			default:
			newDoc.receiptStatusHtml +='invoice-processing">' + doc.receiptAdvice.receiptStatus
			break
		}
		
		
		
		newDoc.receiptStatusHtml +='</span>'
	}
	return newDoc
}
...
exports.makeSimpleInventoryFicheList=function(doc){
	...
	if(doc.docTypeCode=='URETIMECIKIS' || doc.docTypeCode=='URETIMDENGIRIS'){
		...
		// if((doc.productionOrderId || '')!=''){
			if(doc.productionOrderId){
				newDoc.docId +='<span class="badge badge-invoice invoice-iade ml-1">';
