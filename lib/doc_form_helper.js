...
exports.makeSimpleOrderList=function(doc){
	...
	switch(doc.profileId){
		case 'TEMELSIPARIS':	newDoc.orderNo +='status-temel">Temel'; break;
		case 'TICARISIPARIS': newDoc.orderNo +='status-ticari">Ticari';break;
		default: newDoc.orderNo +='status-earsiv">' + doc.profileId + '';break;
	}
	newDoc.orderNo +='</span><span class="badge badge-status ml-1 ';

	switch(doc.orderTypeCode){
		case 'SATIS': newDoc.orderNo +='status-satis">Satış';break;
		case 'IADE': newDoc.orderNo +='status-iade">İade';break;
		case 'TEVKIFAT': newDoc.orderNo +='status-tevkifat">Tevkifat';break;
		case 'ISTISNA':	newDoc.orderNo +='status-istisna">İstisna';break;
		case 'OZELMATRAH': newDoc.orderNo +='status-ozelmatrah">Özel Matrah'; break;
		case 'IHRACKAYITLI': newDoc.orderNo +='status-ihrackayitli">İhraç Kayıtlı';break;
		default: newDoc.orderNo +='status-ihrackayitli">' + doc.orderTypeCode + ''; break;
	}
	newDoc.orderNo +='</span>'

	newDoc.orderStatus ='<span class="badge badge-status ';
	switch(doc.orderStatus){
		case 'Draft': newDoc.orderStatus +='status-draft">Taslak';break;
		case 'Pending': newDoc.orderStatus +='status-pending">Kuyruğa alındı';break;
		case 'Processing': newDoc.orderStatus +='status-processing">İşleniyor';break;
		case 'SentToGib': newDoc.orderStatus +='status-senttogib">GİB\'e iletildi';break;
		case 'Approved':	newDoc.orderStatus +='status-approved">Onaylandı';break;
		case 'Declined': newDoc.orderStatus +='status-declined">Reddedildi'; break;
		case 'WaitingForApprovement': newDoc.orderStatus +='status-waitingforapprovement">Onay bekliyor';break;
		case 'Error': newDoc.orderStatus +='status-error"><a href="javascript:showErrors(\'' + doc._id + '\');"><i class="fa fa-eye"></i> Hata</a>';break;
		default: newDoc.orderStatus +='status-processing">' + doc.orderStatus + ''; break;
	}
...
exports.makeSimpleInventoryFicheList=function(doc){
	...
	switch(doc.docTypeCode){
		case 'TRANSFER':	newDoc.docId +='status-temel">Transfer'; break;
		case 'GIRIS':	newDoc.docId +='status-iade">Giriş'; break;
		case 'CIKIS':	newDoc.docId +='status-satis">Çıkış'; break;
		case 'SAYIMFAZLASI': newDoc.docId +='status-ihrackayitli">Sayım Fazlası(-)';break;
		case 'SAYIMEKSIGI': newDoc.docId +='status-istisna">Sayım Eksiği(+)';break;
		case 'URETIMECIKIS': newDoc.docId +='status-ozelmatrah">Üretime Çıkış';break;
		case 'URETIMDENGIRIS': newDoc.docId +='status-ticari">Üretimden Giriş';break;
		case 'SAYIM': newDoc.docId +='status-tevkifat">Sayım';break;
	}
	newDoc.docId +='</span>';
	...
	exports.makeSimpleProductionOrderList=function(req, doc){
		...
		switch(doc.productionTypeCode){
			case 'DEPO':	newDoc.productionId +='status-temel">Depo'; break;
			default: newDoc.productionId +='status-ticari">' + doc.productionTypeCode + '';break;
		}
		...
		switch(doc.status){
			case 'Draft': newDoc.status +='status-draft">Taslak';break;
			case 'Approved':	newDoc.status +='status-approved">Onaylandı';break;
			case 'Declined': newDoc.status +='status-declined">Reddedildi'; break;
			case 'Processing': newDoc.status +='status-processing">Üretiliyor';break;
			case 'Completed': newDoc.status +='status-senttogib">Bitti';break;
			case 'Cancelled': newDoc.status +='status-earsiv">İptal';break;
			case 'Error': newDoc.status +='status-error"><a href="javascript:showErrors(\'' + doc._id + '\');"><i class="fa fa-eye"></i> Hata</a>';break;
			default: newDoc.status +='status-processing">' + doc.status + ''; break;
		}
		newDoc.status +='</span>';
