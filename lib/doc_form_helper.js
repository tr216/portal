...
exports.makeSimpleDespatchList=function(doc){
	...
	try{
		if(doc.despatchReceiptAdvice._id!=''){
			newDoc.partyName +=`<br>TMiktar:<b>${doc.totalDeliveredQuantity}</b> | <b class="text-primary">Kabul:${doc.despatchReceiptAdvice.totalReceivedQuantity}</b> / <b class="text-danger">Red:${doc.despatchReceiptAdvice.totalRejectedQuantity}</b>`
		}else{
	...
	switch(doc.despatchStatus){
		...
		default: newDoc.despatchStatusHtml +='invoice-processing">' + doc.despatchStatusHtml + ''; break;
	}
