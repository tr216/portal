	...
	function start(fields,options,uiParams,data){
		...
		$(document).ready(()=>{
			...
			Object.keys(fields).forEach((key)=>{
				if((fields[key].type || '').toLowerCase()=='daterange'){
					script +=`
					$('#${formBuilder.generateFormId(key)} #cboEasyDate').on('change',cboEasyDate_onchange)
					if(pageSettings.getItem('${formBuilder.generateFormId(key)}')){
						$('#${formBuilder.generateFormId(key)} #cboEasyDate').val(pageSettings.getItem('${formBuilder.generateFormId(key)}'))
						cboEasyDate_onchange()
					}

					function cboEasyDate_onchange(){
						var obj=cboEasyDateChange($('#${formBuilder.generateFormId(key)} #cboEasyDate').val())

						$('#${formBuilder.generateFormId(key)} #date1').val(obj.date1)
						$('#${formBuilder.generateFormId(key)} #date2').val(obj.date2)
						pageSettings.setItem('${formBuilder.generateFormId(key)}',$('#${formBuilder.generateFormId(key)} #cboEasyDate').val())
					}
					`
				}
			})
		...
		function generateControls(fields,options,uiParams,data,inline=false){
			...
			Object.keys(fields).forEach((key)=>{
				...
				if(item.hasChildren){
					if(frontEnd){
						s+=`<div class="card ${item.col} p-0 m-0 mb-2">
