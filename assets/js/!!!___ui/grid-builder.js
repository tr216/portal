(function(exports) {
	...
	function generateGrid(fields,options,uiParams,data,callback){
		...
		iteration(Object.keys(fields),(key,cb)=>{
			if(fields[key].type=='remoteLookup' && fields[key].dataSource!=undefined && !options.insideForm){
				//cb(null)
				if(frontEnd){
					cb(null)
				}else{
		...
		Object.keys(fields).forEach((key,index)=>{
			...
			if((fields[key].visible==undefined?true:fields[key].visible)){
				...
				switch(item.type.toLowerCase()) {
					...
					case 'remotelookup' : 
					s+=formBuilder.remoteLookup(item,itemValue,true,row)
					break
					case 'money' :
					case 'number' :
					case 'numberbox' :
					...
					s+=formBuilder.lookUp(item,itemValue,true,row)
					break
	...
	function gridRow(id,fields,options,row,rowIndex=-100){
		...
		Object.keys(fields).forEach((key,index)=>{
			if((fields[key].visible==undefined?true:fields[key].visible)){
				...
				switch(fields[key].type.toLowerCase()){
					case 'lookup':
					var lookupText=''
