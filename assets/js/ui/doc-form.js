...
function saveDocument(callback){
   ...
   //  		if($("select[name='location2']").val()!='')	doc.location2=$("select[name='location2']").val().split(';')[0];
	...
	function removeLine(index){
		switch(docFormType){
			case 'order':
			if(index<0 || index>doc.orderLine.length-1) 
				return
			confirmX('Satiri silmek istediginizden emin misiniz?',(resp)=>{
				if(!resp)
					return
			...
			case 'invoice':
			if(index<0 || index>doc.invoiceLine.length-1)
				return;
			confirmX('Satiri silmek istediginizden emin misiniz?',(resp)=>{
				if(!resp)
					return
			...
			break
			case 'despatch':
			if(index<0 || index>doc.despatchLine.length-1)
				return
			confirmX('Satiri silmek istediginizden emin misiniz?',(resp)=>{
				if(!resp)
					return
