	...
	function generateGrid(fields,options,data){
		...
		<div class="table-responsive mt-1">
			...
			data.list.forEach((row,index)=>{
				s+=`<tr>
				${options.selection && row._id?'<td><input type="checkbox" class="grid-checkbox checkSingle" value="${encodeURIComponent2(JSON.stringify(row))}" name="selected[${index}]"  id="selected[${index}]" title="Seç"></td>':''}
		...
		$(document).ready(()=>{
			...
			function runFilter(){
				...
				var filtreVar=false
				fields.forEach((field,index)=>{
					field.filterField = field.filterField || field.field
					if(options.filter){
						filtreVar=true;
				...
				if(filtreVar){
					...
					for(let key in params){
						...
						queryString +=key + '=' + encodeURIComponent2(params[key])
					}
	...
	function gridHeader(fields,options){
		...
		if(options.filter){
			...
			fields.forEach((e,index)=>{
				if(!e.visible){
					e.filterField=e.filterField || e.field
					if(options.filter){
						e.filter=e.filter==undefined?true:e.filter
						e.filterValue=q[e.filterField] || ''
					}
					s+=`<th class="" style="">`
					if(e.filter){
						if(e.type=='lookup'){
