...
// }catch(err){
		...
		function generateControls(item,data,bRoot=false,insideOfModal=false){
			...
			if(item.fields){
				Object.keys(item.fields).forEach((key)=>{
					...
					item.fields[key]=itemDefaultValues(item.fields[key],autocol,insideOfModal)
					if(item.type=='grid'){
				...
				item.tabs.forEach((tab)=>{
					if(tab.fields){
						Object.keys(tab.fields).forEach((key)=>{
							...
							tab.fields[key]=itemDefaultValues(tab.fields[key],autocol,insideOfModal)
						})
					}
				})
			}
			...
			switch((item.type || '').toLowerCase()){
				...
				case 'filter':
				if(item.fields){
					var hashObj=getHashObject(true)
		...
		function gridButtonOptions(item,bRoot,insideOfModal){
			...
			if(options.buttons.add[0]==true && options.buttons.add[1]==''){
				if(bRoot){
					options.buttons.add[1]=`<a href="${menuLink(hashObj.path + '/addnew',{mid:hashObj.query.mid})}" class="btn btn-primary  btn-sm far fa-plus-square" target="_self"  title="Yeni Ekle"></a>`
				}else{
			...
			if(options.buttons.print[0]==true && options.buttons.print[1]==''){
				options.buttons.print[1]=`<a href="${menuLink(hashObj.path + '/print/{_id}',{mid:hashObj.query.mid})}','Yazdır','900','600')" class="btn btn-grid-row btn-info " title="Yazdır"><i class="fas fa-print"></i></a>`
			}

			if(options.buttons.view[0]==true && options.buttons.view[1]==''){
				options.buttons.view[1]=`<a href="${menuLink(hashObj.path + '/view/{_id}',{mid:hashObj.query.mid})}','İncele','900','600')" class="btn btn-info btn-grid-row fas fa-eye" title="İncele"></a>`
			}

			if(options.buttons.edit[0]==true && options.buttons.edit[1]==''){
				if(bRoot){
					options.buttons.edit[1]=`<a href="${menuLink(hashObj.path + '/edit/{_id}',{mid:hashObj.query.mid})}" class="btn btn-primary btn-grid-row fas fa-edit" target="_self"  title="Düzenle"></a>`
				}else{
					if(item.modal && !item.insideOfModal){
		...
		function itemDefaultValues(item,autocol=false,insideOfModal=false){
